...
class FCRBM(tf.Module):
    ...
    def _visible_mean(self, h, u):
        # calculate dynamic visible biases
        dynamic_visible_bias = self.visible_bias + tf.linalg.matmul(u, self.W_vu, transpose_b=True)
        # The reconstruction term from the hidden units
        reconstruction_term = tf.linalg.matmul(h, self.W_vh, transpose_b=True)
        return dynamic_visible_bias + reconstruction_term # mean for a gaussian visible layer
