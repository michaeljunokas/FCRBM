...
epsilonvisfac=single(1e-7);
%only one set of featfac parameters
%shared between undirected, A & B models
epsilonfeatfac=single(1e-7);
epsilonhidfac=single(1e-7);
...
epsilonpastfacB=single(1e-7);
epsilonhidfacB=single(1e-7);

epsilonlabelfeat=single(1e-7);

epsilonvisbias=single(1e-7);
epsilonhidbias=single(1e-7);
