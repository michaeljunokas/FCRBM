function [ FCRBMConfig ] = train_binaryFCRBMmikeTESTnot3(FCRBMConfig)
...
epsilonvisfac=1e-2;     %Corresponding to the "undirected" observation model
epsilonfeatfac=1e-2;    %only one set of featfac parameters...
epsilonhidfac=1e-2;     %shared between undirected, A & B models

epsilonpastfacA=1e-2;   %Corresponding to the "directed" Autoregressive model
epsilonfeatfacA=1e-2;
epsilonvisfacA=1e-2;

epsilonpastfacB=1e-2; %Corresponding to the "directed" past->hidden model
epsilonfeatfacB=single(1e-2);
epsilonhidfacB=1e-2;

epsilonlabelfeat=1e-2;
epsilonvisbias=1e-2;
epsilonhidbias=1e-2;
...
labelfeat = noise*rand(numlabels,numfeat);  
...
% % % 
% % % for ii = 1:length(FCRBMConfig.classes)
% % %     classLength(ii) = size(FCRBMConfig.classes{ii},1);
% % %     if ii == 1
...
for epoch = 1:FCRBMConfig.numepochs,
  ...
  %%%%%%%%% START POSITIVE PHASE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    numcases = length(FCRBMConfig.data); %%was cases
...
%         
 ...
 negdata2 = 1./(1+exp(-negdata));
       negdata = single(negdata2 > rand(numcases,numdims));
