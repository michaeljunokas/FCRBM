function [ FCRBMConfig ] = MikeTestFCRBM(FCRBMConfig)
...
epsilonvisfac=1e-1;     %Corresponding to the "undirected" observation model
epsilonfeatfac=1e-1;    %only one set of featfac parameters...
epsilonhidfac=1e-1;     %shared between undirected, A & B models

epsilonpastfacA=1e-1;   %Corresponding to the "directed" Autoregressive model
epsilonfeatfacA=1e-1;
epsilonvisfacA=1e-1;

epsilonpastfacB=1e-1; %Corresponding to the "directed" past->hidden model
epsilonfeatfacB=1e-1;
epsilonhidfacB=1e-1;

epsilonlabelfeat=1e-1;
epsilonvisbias=1e-1;
epsilonhidbias=1e-1;
...
%         (ypasthidB_)*featfac';
      ...
      hidstates = single(neghidprobs > rand(numcases,numhid));      %%% ADDED
      yhid = hidstates*hidfac;                                         %%% ADDED
