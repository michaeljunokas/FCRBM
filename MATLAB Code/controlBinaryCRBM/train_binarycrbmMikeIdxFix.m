function [ CRBMConfig ] = train_binarycrbmMikeIdxFix( CRBMConfig)
...
% % % % % % % % % %     
% % % % % % % % % %     
% % % % % % % % % % %     nffigure(ffff);imagesc(filteringdist', [0 1]); colormap gray;
% % % % % % % % % % %     title(['Hidden Unit Activations During Training - ', CRBMTitle]); ylabel('hidden units'); xlabel('frames / samples')
% % % % % % % % % % %     
% % % % % % % % % % end
%%%%%%%%%%%%%%%%
% % % % 
% % % % for index = 0:classIndex(1)-CRBMConfig.order
% % % %     data(:,:,1) = CRBMConfig.data(classIndex-index,:)
% % % %      for hh=1:CRBMConfig.order
...
for index = 1:classIndex(1)-CRBMConfig.order
    data = zeros(numcases,numdims,CRBMConfig.order+1);
    pointer = classIndex-index+1;
    data(:,:,1) = CRBMConfig.data(pointer,:);
    for hh=1:CRBMConfig.order
      data(:,:,hh+1) = CRBMConfig.data(pointer-hh,:);
    end
...
% SAYS WE DON'T NEED IT, CAN JUST USE PROBABILITIES DIRECTLY....
     visstates = double(vposteriors > rand(numcases,numdims));   
  
    %Calculate negative gradients                                   
    negwgrad = hidstates'*visstates;
    negbigrad = sum(visstates' - ...
        repmat(bi,1,numcases) - bistar,2);
    negbjgrad = sum(hidstates,1)';
           
    for hh=1:CRBMConfig.order      
      negAgrad(:,:,hh) = (visstates' -  ...
        repmat(bi,1,numcases) - bistar) * data(:,:,hh+1);
...
%%%%%%%%% END NEGATIVE PHASE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     err= sum(sum( (data(:,:,1)-visstates).^2 ));
     errsum = err + errsum;
